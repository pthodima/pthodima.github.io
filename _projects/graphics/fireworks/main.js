...
let box = /** @type {DOMRect} */ canvas.getBoundingClientRect();
...
canvas.onclick = function (/** @type {MouseEvent} */ event) {
    // Use offsetX/Y which are relative to the padding edge (content box), automatically handling borders.
    // Scale against clientWidth/Height to match the internal canvas resolution.
...
function animateFireworks(/** @type {DOMHighResTimeStamp} */ timestamp) {
    ...
    fireworks.forEach(function (firework) {
        ...
        // We use the current velocity 'firework.vy' for the time step
    ...
    // request next frame if there are still fireworks or particles
